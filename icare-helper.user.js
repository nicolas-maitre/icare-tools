...
if (window.location.href.includes("PersonenList.do")) {
    ...
    const links = [...document.querySelectorAll("a")].filter((e) => e.href.includes("/icare/Be/PersonEdit.do?method=show&perEdit="));
...
if (window.location.href.includes("PersonEdit.do?method=show&perEdit=")) {
    ...
    //auto contracts
    const contracts = [...document.querySelectorAll("a")].filter((e) => e.href.includes("Be/VertragEdit.do?method=main&aktuelle=true&theVerId="))
...
if (window.location.href.includes("VertragList.do")) {
    ...
    function updateHighlight(name, val) {
        ...
        TRs.forEach(tr => {
            ...
            if (checked)
                ...
                tr.classList.remove("highlightedRow")
        });
    }
    /**
    ...
    function getTrData(tr) {
        const name = tr.querySelector("tr > td > span > a")?.textContent?.trim()
            // ?.split("-")?.[0] 
            ?? "_";
        /** @type {HTMLInputElement | null} */
...
if (window.location.href.includes("VertragEdit.do")) {
    ...
    //     const printTabLink = await waitForSelector("ul > li[role=tab] > a#ui-id-11", 250);
    //     printTabLink?.click();
    // })();
